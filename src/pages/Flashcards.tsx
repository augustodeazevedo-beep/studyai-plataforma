...
import { Plus, RotateCcw, Trash2, BookOpen, Check, X } from "lucide-react";
    ...
    return (
      <div className="max-w-xl mx-auto space-y-6 py-8">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">{current + 1} / {studyQueue.length}</span>
        </div>
        <div
    ...
    return (
      <div className="max-w-xl mx-auto space-y-4 py-8">
        ...
        <div className="space-y-2">
          ...
          <Textarea placeholder="Ex: Qual o conceito de ITCMD?" value={newFront} onChange={(e) => setNewFront(e.target.value)} rows={3} />
        </div>
        <div className="space-y-2">
          <label className="text-xs uppercase tracking-wide text-muted-foreground">Verso (resposta)</label>
          <Textarea placeholder="Ex: Imposto sobre transmissão..." value={newBack} onChange={(e) => setNewBack(e.target.value)} rows={3} />
        </div>
        <Button onClick={createCard} className="w-full">Criar Cartão</Button>
      </div>
    );
  }
