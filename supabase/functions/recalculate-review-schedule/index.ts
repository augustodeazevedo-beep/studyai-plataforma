...
async function recalculateForUser(supabase: any, userId: string, payload: z.infer<typeof bodySchema>) {
  const [subjectsRes, topicsRes, sessionsRes, reviewsRes, planRes, profileRes, checkinsRes] = await Promise.all([
    ...
    supabase.from("topics").select("id, subject_id, name, completed").eq("user_id", userId),
    supabase.from("study_sessions").select("id, subject_id, topic_id, started_at, duration_minutes, comprehension_rating").eq("user_id", userId),
    supabase.from("spaced_reviews").select("id, subject_id, topic_id, review_date, completed, performance_rating, interval_days").eq("user_id", userId),
    supabase.from("study_plan").select("subject_id, priority_score, relevance, incidence, gap_score, performance").eq("user_id", userId),
  ...
  for (const target of targets) {
    ...
    if (payload.subjectId && target.subject.id !== payload.subjectId) continue;
    if (payload.topicId && target.topic?.id !== payload.topicId) continue;
